...
impl<const N: usize> SerBuffer<N> {
    ...
    pub(super) fn write_up_to(
        ...
        // dust-stark patch 2026-05-22: was `num_bytes > 8 * (N - 1)`.
        // Over-strict by one: when num_bytes == 8 * (N - 1) the body
        // computes remaining_bytes = 0 and writes &buffers[N-1][..0]
        // — a valid no-op, not an underflow.  For N == 1 the original
        // assert also rejects num_bytes == 0, which is a legitimate
        // "serialize nothing" call.  Relaxing > to >= preserves the
        // underflow guard (num_bytes < 8 * (N - 1) is still rejected
        // for N >= 2) without breaking 1-limb Goldilocks paths.
    ...
    pub(super) fn read_exact_up_to(
        ...
        debug_assert!(num_bytes <= 8 * N + 1, "index too large");
        // dust-stark patch 2026-05-22: was `num_bytes > 8 * (N - 1)`.
        // Same reasoning as write_up_to above — the assert was
        // over-strict by one and excluded the lower bound the
        // method's own doc-comment claims is valid
        // ("allowed to range from 8 * (N - 1) to 8 * N + 1").
        debug_assert!(num_bytes >= 8 * (N - 1), "index too small");
        // unconditionally write first `N - 1` limbs.
        for i in 0..(N - 1) {
