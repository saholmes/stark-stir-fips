...
// ─────────────────────────────────────────────────────────────────────────────
//  Disk-spilled (file-backed) implementation — the IoT path
// ─────────────────────────────────────────────────────────────────────────────

#[cfg(not(target_arch = "wasm32"))]
pub use spill::SpillFpColumn;

/// File-backed [`FpColumnRead`]: one column's `n` field elements stored
/// as little-endian Goldilocks `u64`s, read via seek on a cached
/// handle.  A streaming composer over `SpillFpColumn`s never resides
/// the `O(n · width)` LDE — peak RAM is the per-column file handles
/// (`O(width)`) + the composer's own buffers.  Plain `std::fs` (no
/// mmap crate); `get` is a seek+read on a `RefCell<File>` handle (the
/// handle is cached — re-opening per `get` is catastrophic, cf. the
/// W8 `DiskLdeStore` fix).  Off wasm.
#[cfg(not(target_arch = "wasm32"))]
mod spill {
    use super::{FpColumnRead, F};
    ...
    const FB: usize = 8; // Goldilocks: one u64 limb.
    static COUNTER: AtomicU64 = AtomicU64::new(0);

    #[inline]
    fn to_le(v: F) -> [u8; FB] {
        v.into_bigint().0[0].to_le_bytes()
    }
    #[inline]
    fn from_le(b: &[u8]) -> F {
        let mut a = [0u8; FB];
        a.copy_from_slice(&b[..FB]);
        F::from(u64::from_le_bytes(a))
    }

    /// A single LDE column spilled to a backing file.
    pub struct SpillFpColumn {
        len: usize,
        path: PathBuf,
        file: RefCell<File>,
    }
    ...
    impl SpillFpColumn {
        /// Write `values` to a fresh file under `dir` and return a
        /// read-only, seek-backed column.  The producer that calls
        /// this one column at a time (then drops the source `Vec`)
        /// never holds the full LDE resident.
        pub fn spill(values: &[F], dir: impl Into<PathBuf>) -> std::io::Result<Self> {
            let dir = dir.into();
            std::fs::create_dir_all(&dir)?;
            let id = COUNTER.fetch_add(1, Ordering::Relaxed);
            let path = dir.join(format!("col-{}-{id}.bin", std::process::id()));
            {
            ...
            Ok(Self { len: values.len(), path, file })
        }
    }

    impl FpColumnRead for SpillFpColumn {
        fn len(&self) -> usize {
            self.len
        }
        fn get(&self, idx: usize) -> F {
            let mut f = self.file.borrow_mut();
            f.seek(SeekFrom::Start((idx * FB) as u64))
                .expect("SpillFpColumn seek");
            let mut b = [0u8; FB];
            f.read_exact(&mut b).expect("SpillFpColumn read");
            from_le(&b)
        }
